...
class SmartSelect {
  ...
  init() {
    // 1. จัดการค่าเริ่มต้นก่อนการ Render
    this.handlePreSelection()
    // 2. สร้างโครงสร้าง DOM
    this.renderBaseStructure()
    this.cacheElements()
    this.bindEvents()
    // 3. แสดงผลค่าเริ่มต้นใน UI
    this.updateUI()
  }
  ...
  renderBaseStructure() {
    this.container.innerHTML = `
      <div class="smart-select-root relative w-full">
          <div class="input-wrapper min-h-12 w-full flex items-center px-4 py-2 bg-white border border-gray-200 rounded-xl shadow-sm focus-within:ring-4 focus-within:ring-emerald-50 focus-within:border-emerald-500 transition-all cursor-text">
              <div class="flex flex-wrap items-center gap-2 flex-1">
                  <div class="chips-list flex flex-wrap gap-2"></div>
                  <input id="${this.inputId}" type="text" class="search-input flex-1 min-w-15 outline-none bg-transparent text-sm text-gray-700 py-1" placeholder="${this.config.placeholder}" autocomplete="off">
              </div>
              <i class="fa-solid fa-chevron-down chevron-icon text-gray-300 text-[10px] transition-transform duration-300 ml-2"></i>
          </div>
          <div class="dropdown-menu hidden absolute z-50 w-full mt-2 bg-white border border-gray-100 rounded-2xl shadow-2xl overflow-hidden">
              <ul class="options-list overflow-y-auto custom-scrollbar p-2" style="max-height: ${this.config.maxHeight}"></ul>
              <div class="no-data hidden p-6 text-center text-sm text-gray-400 italic font-light">ไม่พบผลลัพธ์</div>
          </div>
      </div>`
  }
  ...
  filterOptions(query = '') {
    ...
    if (this.filteredOptions.length === 0) {
      ...
      this.filteredOptions.forEach((opt, idx) => {
        ...
        li.className = `px-4 py-2.5 text-sm cursor-pointer rounded-xl transition-all flex items-center justify-between group ${idx === 0 ? 'option-highlighted' : ''}`
  ...
  selectOption(opt) {
    ...
    if (this.config.multiple) {
      this.selectedValues.push(valStr)
    } else {
      this.selectedValues = [valStr]
      this.closeDropdown()
    }
  ...
  updateUI() {
    ...
    this.selectedValues.forEach((val) => {
      ...
      chip.className =
        'flex items-center gap-1.5 px-3 py-1 bg-emerald-600 text-white text-[12px] font-medium rounded-lg animate-in zoom-in-95 duration-200 shadow-sm'
